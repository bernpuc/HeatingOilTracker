...
public class ChartsViewModel : INotifyPropertyChanged
{
    ...
    public bool HasKFactorData { get => _hasKFactorData; set => SetProperty(ref _hasKFactorData, value); }
    public bool HasDeliveryComparisonData { get => _hasDeliveryComparisonData; set => SetProperty(ref _hasDeliveryComparisonData, value); }
    public ObservableCollection<ISeries> BurnRateSeries { get => _burnRateSeries; set => SetProperty(ref _burnRateSeries, value); }
    public ObservableCollection<ISeries> PriceSeries { get => _priceSeries; set => SetProperty(ref _priceSeries, value); }
    public ObservableCollection<ISeries> KFactorSeries { get => _kFactorSeries; set => SetProperty(ref _kFactorSeries, value); }
    public ObservableCollection<ISeries> DeliveryComparisonSeries { get => _deliveryComparisonSeries; set => SetProperty(ref _deliveryComparisonSeries, value); }
    public Axis[] BurnRateXAxes { get => _burnRateXAxes; set => SetProperty(ref _burnRateXAxes, value); }
    public Axis[] BurnRateYAxes { get => _burnRateYAxes; set => SetProperty(ref _burnRateYAxes, value); }
    public Axis[] PriceXAxes { get => _priceXAxes; set => SetProperty(ref _priceXAxes, value); }
    public Axis[] PriceYAxes { get => _priceYAxes; set => SetProperty(ref _priceYAxes, value); }
    public Axis[] KFactorXAxes { get => _kFactorXAxes; set => SetProperty(ref _kFactorXAxes, value); }
    public Axis[] KFactorYAxes { get => _kFactorYAxes; set => SetProperty(ref _kFactorYAxes, value); }
    public Axis[] DeliveryComparisonXAxes { get => _deliveryComparisonXAxes; set => SetProperty(ref _deliveryComparisonXAxes, value); }
    public Axis[] DeliveryComparisonYAxes { get => _deliveryComparisonYAxes; set => SetProperty(ref _deliveryComparisonYAxes, value); }
    ...
    public async Task LoadChartsAsync()
    {
        ...
        for (int i = 1; i < sorted.Count; i++)
        {
            ...
            if (days > 0)
            {
                ...
                if (isOffSeasonDelivery) offSeasonRates.Add(rate);
                else if (month >= 10 || month <= 3) heatingSeasonRates.Add(rate);
            }
        }
        ...
        if (weatherData.Count > 0 && kFactorPoints.Count > 0)
        {
            ...
            for (int i = 1; i < sorted.Count; i++)
            {
                ...
                if (avgKFactor > 0 && hdd > 0)
                    estimatedHddPoints.Add(new DateTimePoint(sorted[i].Date, (double)hdd / avgKFactor));
                else
                ...
                {
                    var m = current.Month;
                    if (m >= 4 && m <= 9) offSeasonDays++;
                    else heatingSeasonDays++;
                    current = current.AddDays(1);
                }

                if ((avgOffSeasonBurnRate > 0 || avgHeatingSeasonBurnRate > 0) && days > 0)
                    estimatedBurnRatePoints.Add(new DateTimePoint(sorted[i].Date,
                        (avgOffSeasonBurnRate * offSeasonDays) + (avgHeatingSeasonBurnRate * heatingSeasonDays)));
                else
